...
namespace GEM.Model
{
	public class Task
	{
		...
		public Site OutdatedSurvey { get; internal set; }// When Fetcher signals to Tasker that new Geodata was just pulled from the API, Tasker class will only update this property if a Tasker template pattern was executed in full
		public Site UpdatedSurvey { get; internal set; } = (Site)Fetcher.Survey.Clone(); // When Fetcher signals to Tasker that new Geodata was just pulled from the API, Tasker class will update this property with the newer Fetcher.Survey property
